...
class ClientOrderForm extends React.Component{
    ...
    getClientList = () => {
        fetch('https://solucionesoggk.com/api/v1/clients_resumed', {
            method: 'GET',
            headers: {
    ...
    onSubmit = (id) => {
        ...
        if(!valid_form){
            ...
            this.orderFormTwo.append('by_client','1');
            fetch('https://solucionesoggk.com/api/v1/add_sale_order', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'multipart/form-data',
                    },
                    body: this.orderFormTwo,
                        ...
                        fetch('https://solucionesoggk.com/api/v1/id_orden_ventah?numeracion='+res.data.numeracion, {
                                    method: 'GET',
                                    headers: {
                                        'Content-Type': 'multipart/form-data',
                                        'Cache-Control': 'no-cache'
                                        ...
                                        if (res.success === true){
                                            this.state.selected_products.map( (item) => {
                                                ...
                                                fetch('https://solucionesoggk.com/api/v1/add_sale_order_detail', {
                                                        method: 'POST',
                                                        headers: {
                                                            'Content-Type': 'multipart/form-data',
                                                            'Cache-Control': 'no-cache',
                        ...
                        (
                            ...
                            <Text>Hubo un error, no figura ningun producto seleccionado</Text>
                                ...
                                <View style={{flex:2,width: 100, height: 20, backgroundColor: 'white'}}>
                                 ...
                                 <View style={{flex: 1, flexDirection: 'row', marginBottom:5, marginTop:5}}>
                                <View style={{flex:2,width: 100, height: 20}}>
                                    <Text style={{fontWeight: 'bold', fontStyle:'italic', color:'grey'}}>{parseFloat(this.state.totals.sub_total).toFixed(2)}</Text>
                                 </View> 
                                <View style={{flex:2,width: 100, height: 20}}>
                                    <Text style={{fontWeight: 'bold',fontStyle:'italic', color:'grey'}}>{parseFloat(this.state.totals.igv).toFixed(2)}</Text>
                                 </View> 
                                <View style={{flex:2,width: 100, height: 20, backgroundColor: 'white'}}>
                                    <Text style={{fontWeight: 'bold', fontStyle:'italic', color:'grey'}}>{parseFloat(this.state.totals.total).toFixed(2)}</Text>
                                </View>
                                </View>  
                </Card>

                    <TouchableOpacity style={styles.buttonStyle} onPress={() => this.onSubmit(this.state.current_user.id)}> 
                        <Text style={styles.textButton}>Guardar Orden de venta</Text>
                    </TouchableOpacity>
